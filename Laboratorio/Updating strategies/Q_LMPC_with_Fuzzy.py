...
class Q_LMPC():
    ...
    def __init__(self, Device_, ensemble_NN_, actor_NN_, critic_NN_, ensemble_size_, prediction_horizon_, buffer_size_, samples_num_):
        ...
        self.PBO_record = np.zeros((self.buffer_size, 1))
        self.time_record = np.zeros((self.buffer_size, 1))
    ...
    def fuzzy_logic(self, cartesian_velocity_, wrench_, dwrench_, PBO_index_,range_index):
        ...
        self.Al_med_min = 0.025 + 0.005*PBO_index_
        self.Al_med_max = 0.045 + 0.005*PBO_index_
        self.Al_high_min = 0.040 + 0.005*PBO_index_
        self.Al_high_max = 0.070 + 0.005*PBO_index_
        ...
        AL_sim.compute()
        AL_= AL_sim.output['Assistance Level']
        #rospy.loginfo(f"AL given: {AL_}")
        return AL_ 
    ...
    def cost_func_p(self, fh_, delta_u_): 
        ...
        cost_ = (np.sum(np.sum(np.multiply(fh_,fh_)*Q_, axis=2).reshape(ense_, N_), axis=0)/ense_).reshape(N_) + (np.sum(np.sum(np.multiply(delta_u_,delta_u_)*Q_, axis=2).reshape(ense_, N_), axis=0)/ense_).reshape(N_)
    ...
    def Critic_train(self, state_force_train, u_record_train_, timestamp, PBO_record, Critic_NN, xy_norm, Cost_norm, Nh, learning_rate):
        ...
        u_unorm_ = (u_record_train_*u_std_)+u_mean_
        vel_unorm_ = (state_force_train[0:Nh_,0:1]*vel_std_)+vel_mean_
        pose_unorm_ = (state_force_train[0:Nh_,1:2]*pose_std_)+pose_mean_
        fh_unorm_ = (state_force_train[0:Nh_,2:3]*fh_std_)+fh_mean_
        ...
        fuzzy_delta_u_= np.zeros((Nh_-1,1))
        for i in range(Nh_-1):
            u_range_ = np.zeros(4)
            for j in range(4):
                u_range_[j] = self.fuzzy_logic(abs(vel_unorm_[i,0]),abs(fh_unorm_[i,0]),abs(dft_filtered[i,0]),PBO_record_[i,0],j)*self.shaping_function(abs(vel_unorm_[i,0]),j)
            fuzzy_delta_u_[i,0] = np.sum(u_range_)*np.sign(dft_filtered[i,0])
        
        if  fuzzy_delta_u_>0.08:
            fuzzy_delta_u_=0.08
        elif fuzzy_delta_u_< -0.08:                
            fuzzy_delta_u_=-0.08
            #This only computes the difference between the current position and the desired next position

        fuzzy_u_ = fuzzy_delta_u_+ pose_unorm_[0:Nh-1,:]
        delta_setpoint_unorm_ = fuzzy_u_ - u_unorm_[0:Nh_-1,0:1]

        #Normalize everything
        delta_setpoint_norm_ = (delta_setpoint_unorm_[0:Nh_-2,0:1]- u_mean_)/u_std_
        fh_norm_train = np.copy(state_force_train[0:Nh_-2,2:3])

        delta_setpoint_norm_p1 = (delta_setpoint_unorm_[1:Nh_-1,0:1]- u_mean_)/u_std_
        fh_norm_train_p1 = np.copy(state_force_train[1:Nh_-1,2:3])

        # Convert each NumPy array to a PyTorch tensor
        delta_setpoint_norm_torch = torch.tensor(delta_setpoint_norm_, dtype=torch.float32, device=self.Device)
        fh_norm_train_torch = torch.tensor(fh_norm_train, dtype=torch.float32, device=self.Device)

        delta_setpoint_norm_p1_torch = torch.tensor(delta_setpoint_norm_p1, dtype=torch.float32, device=self.Device)
        fh_norm_train_p1_torch = torch.tensor(fh_norm_train_p1, dtype=torch.float32, device=self.Device)

        # Concatenate the tensors
        combined_input_torch = torch.cat((delta_setpoint_norm_torch, fh_norm_train_torch), dim=1)
        combined_input_p1_torch = torch.cat((delta_setpoint_norm_p1_torch, fh_norm_train_p1_torch), dim=1)
        ...
        for j in range(0, Nh_-1):
            ...
            Cost_fh = (combined_input_torch[j:j+1,0].unsqueeze(0)).detach().numpy()*fh_std_ + fh_mean_
            Cost_du = (combined_input_torch[j:j+1,1].unsqueeze(0)).detach().numpy()*u_std_ + u_mean_
            
            COSTO = self.cost_func_p(Cost_fh,Cost_du)/cost_std_dev # normalization 
            #print("F", Cost_f_0)
            del Cost_fh, Cost_du
    ...
    def CEM_critic(self, x_initial_, action_dim_, time_horizon_, num_samples_, xy_norm_, U_l_U_, L_l_U_, U_l_D_, L_l_D_,
               NN_model_, Critic_NN_, Actor_NN_, PBO_index, num_ensembles_cem_):
        ...
        for _ in range(iteration_):
            ...
            for t in range(time_horizon_):
                ...
                action_tt_norm_torch = Actor_NN.forward(state_tt_norm_torch_.view(num_samples_,state_dim_)) 

                action_tt_norm_= np.asarray(action_tt_norm_torch.detach()).reshape(num_ensembles_, num_samples_, 
                                                                                   action_dim_)
                ...
                for j1 in range(0,num_samples_):
                    ...
                    for j2 in range(0,num_ensembles_cem_):
                        ...
                        velocity_tt = state_tt_[j2,j1,0]
                        position_tt = state_tt_[j2,j1,1]
                        force_tt = state_tt_[j2,j1,2]
                        force_t = state_t_broadcasted_[j2,j1,2]
                        dforce_raw = (force_tt-force_t)*6
                        if t==0:
                            dforce_filt[j2,j1,t]= 0
                        else:
                            dforce_filt[j2,j1,t]=self.low_pass_filter(dforce_raw,dforce_filt[j2,j1,t-1])
                        action_range = np.zeros(4)
                        for i in range(4):
                            action_range[i]=self.fuzzy_logic(velocity_tt,force_tt,dforce_filt[j2,j1,t],PBO_index_,i)*self.shaping_function(velocity_tt,i)

                        fuzzy_action = np.sum(action_range)*np.sign(dforce_filt[j2,j1,t]) + position_tt

                        delta_action_unorm_ = fuzzy_action-action_tt_[j2,j1,0]
                        delta_action_norm_ = (delta_action_unorm_ - x_mean_v_[3])/x_std_v_[3]

                        Critic_input_ = torch.tensor([[state_tt_norm_[j2,j1,2],delta_action_norm_]])

                        costo_rete_ = Critic_NN.forward(Critic_input_)
    ...
    def Actor_train(self, state_force_train, Cost_norm, U_l_U_, L_l_U_, U_l_D_, L_l_D_,
                Actor_NN, Critic_NN, Model_NN, xy_norm, Nh, PBO_index, learning_rate):
        ...
        for j in range(0, Nh_):
            ...
            U_npiu1, D_npiu1 = self.CEM_critic(state_force_train_not_norm_[j+1:j+2], 1, self.prediction_horizon, 
                                               ...
                                               L_l_U_[j:j+1].item(), U_l_D_[j:j+1].item(), L_l_D_[j:j+1].item(), Model_NN_,Critic_NN_,Actor_NN_,PBO_index_[j:j+1,0], num_ensembles_cem_= 1)
    ...
    def measurements_callback(self, pose, velocity, wrench, pbo):
        ...
        if (self.iter_buffer_ < self.buffer_size):
            ...
            # denormalization
            self.u_3 = (u_D_3_norm[0]*self.x_std_v[3] + self.x_mean_v[3]).detach().numpy()
            self.Dr_3 = (u_D_3_norm[1]*self.x_std_v[4] + self.x_mean_v[4]).detach().numpy()
            ...
            state_action_norm = np.append(actor_input_norm, u_D_3_norm[0])
            state_action_damping_norm = np.append(state_action_norm, u_D_3_norm[1].detach().numpy(), axis=1)
        ...
        if (self.iter_buffer_ % self.buffer_size == 0):
            ...
            self.Actor_train(state_force_norm_tra, self.Cost_norm, self.Upper_limit_u_norm,
                             ...
                             self.actor_NN, self.critic_NN, self.NN_ensemble, self.training_dict['xy_norm'], self.buffer_size, self.PBO_record, learning_rate = lr_actor)
